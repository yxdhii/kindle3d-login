...
export function useWebGLBackground(canvasRef) {
    useEffect(() => {
        ...
        uniform vec2 u_res;
        void main() { vec2 uv = gl_FragCoord.xy / u_res; float t = u_time * 0.4;
        vec3 col = vec3(0.10, 0.03, 0.07);
        float d1 = length(uv - vec2(0.3 + sin(t)*0.15, 0.4 + cos(t*0.7)*0.15));
        float d2 = length(uv - vec2(0.7 + cos(t*0.6)*0.12, 0.6 + sin(t*0.8)*0.12));
        col += vec3(0.35, 0.04, 0.18) * (0.18 / (d1 + 0.01));
        col += vec3(0.25, 0.0, 0.12) * (0.14 / (d2 + 0.01));
